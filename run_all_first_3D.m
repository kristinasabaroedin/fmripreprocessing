function [] = run_all_first(subject)
    ...
    % Edited by Linden Parkes, June 2015.
    %
    %==========================================================================

    % clear all;
    % subject = '1008.2.57.25';
    ...
    %==========================================================================
        ...
        % rawdir = ['/media/lindenmp/WD_2TB/Research_Projects/KristinaHons/data/',subject,'/rfMRI/']; % where the unprocessed epi 4d files are
        rawdir = ['/gpfs/M2Home/kristina_s/Monash076/Kristina/GenofCog/data/',subject,'/rfMRI/']; % where the unprocessed epi 4d files are
        ...
        % t1dir = ['/media/lindenmp/WD_2TB/Research_Projects/KristinaHons/data/',subject,'/t1/']; 
        t1dir = ['/gpfs/M2Home/kristina_s/Monash076/Kristina/GenofCog/data/',subject,'/t1/']; 
        % name of t1 file.
        t1name = ['t1.nii'];  
        ...
        roidir = '/gpfs/M2Home/kristina_s/Monash076/Kristina/GenofCog/TriStri/';
    ...
    %==========================================================================
        ...
        % Tissue segment T1 with SPM; can comment out the line below to skip T1 segmentation
    ...
    %==========================================================================
        ...
        % realignment; writes out motion parameters
    ...
    %==========================================================================
        ...
        cd(dtdir)
        system([fsldir,'fslsplit ',DetrendIn])
        system(['mv ./',DetrendIn,' ../'])
        cd(rawdir)
    ...
    %==========================================================================
        ...
        cd(cleandir)
        system([fsldir,'fslsplit detrend_clean.nii'])
        system(['mv ./detrend_clean.nii ../'])
        cd(rawdir)
    ...
    %==========================================================================
        ...
        if ~isempty(kernel)
            ...
            cd(rawdir)
            mkdir('temp')
            copyfile('rest_prepro.nii',[rawdir,'temp'],'f')
            cd([rawdir,'temp'])

            % Split EPI into 3D files
            spm_file_split('rest_prepro.nii')

            % Smooth
            smooth_epi_3D([rawdir,'temp/rest_prepro'],kernel,N);

            % Merge smoothed EPI
            system([fsldir,'fslmerge -t srest_prepro.nii srest_prepro_*.nii'])

            movefile('srest_prepro.nii',rawdir)
            cd(rawdir)
            rmdir([rawdir,'temp'],'s')
    ...
    %==========================================================================
        if run_stats
            ...
            first_level_3D([rawdir,'FirstLevel_L/'],'scans',TR,[rawdir,extract_in],N,[rawdir,'FirstLevel_L/spm_regs_L.mat'],[t1dir,'wt1_brain.nii']);
            % run contrasts
            ...
            first_level_3D([rawdir,'FirstLevel_R/'],'scans',TR,[rawdir,extract_in],N,[rawdir,'FirstLevel_R/spm_regs_R.mat'],[t1dir,'wt1_brain.nii']);
            % run contrasts
    ...
    mkdir('rfMRI_FirstLevel')
    % Copy First level subdirs
    copyfile([rawdir,'FirstLevel_L'],'rfMRI_FirstLevel/FirstLevel_L')
    copyfile([rawdir,'FirstLevel_R'],'rfMRI_FirstLevel/FirstLevel_R')
